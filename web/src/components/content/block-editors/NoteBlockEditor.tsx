...
const NoteBlockEditor: React.FC<NoteBlockEditorProps> = ({ block, onChange }) => {
  ...
  const handleChange = (editorState: any) => {
    onChange(block.id, { ...block.data, content: JSON.stringify(editorState) });
  };
      ...
      <div className="w-full px-3 py-2 bg-dark-800 border border-dark-600 rounded text-white text-sm focus-within:outline-none focus-within:ring-1 focus-within:ring-primary-500 min-h-[200px]">
        <LexicalComposer initialConfig={initialConfig}>
          ...
          <RichTextPlugin
            contentEditable={
              <ContentEditable
                className="w-full outline-none min-h-[150px] px-1 py-1"
                aria-placeholder="Note content..."
                placeholder={
